...
add('1-A',1,'朝の手順','「靴を履く→靴下を履く→外へ出る」をやり直しが起きない順序に直してください。また、このように上から決められた順序で一つずつ進む処理を、三つの基本構造で何と呼びますか。',
 ['靴下を履く','靴を履く','外へ出る'],'靴下を履く→靴を履く→外へ出る。上から順に進む処理は「順次」です。逆に戻ったり飛ばしたりせず、書かれた順番に一行ずつ実行します。',null,{natural:true,answerFields:['正しい順序','基本構造の名称'],aiQuestion:'問題1-Aについて質問です。手順の順番を入れ替える考え方と、これが「順次」構造と呼ばれる理由を確認してください。'});
add('1-B',1,'雨の日の準備','「出かける前に空を見て、雨なら傘を持ち、晴れなら帽子をかぶる」という処理があります。条件によって進む道を変える処理を、三つの基本構造で何と呼びますか。また、この例での条件は何ですか。',
 [branch('雨が降っている',['傘を持つ'],['帽子をかぶる'])],'条件で分ける処理は「選択」です。条件は「雨が降っているかどうか」です。条件が「真（成立）」か「偽（不成立）」かで、実行する処理を選びます。',null,{natural:true,answerFields:['基本構造の名称','この例での条件'],aiQuestion:'問題1-Bについて質問です。天候による行動の分岐が「選択」構造に当たる理由と、条件の整理方法を確認してください。'});
add('1-C',2,'同じ作業の繰り返し','「10枚の書類すべてに確認印を押す」という作業があります。三つの基本構造のどれに当たりますか。また、この処理が終わるのはどんな条件のときですか。',
 [repeat('書類','1','10',['確認印を押す'])],'同じ処理を繰り返す「繰返し」です。終了条件は「10枚すべてに押し終わったとき（残り0枚）」です。回数が決まっている場合と、条件を満たすまで続ける場合があります。',null,{natural:true,answerFields:['基本構造の名称','終了する条件'],aiQuestion:'問題1-Cについて質問です。同じ動作を繰り返す処理の終了条件の捉え方と、「繰返し」構造の役割を確認してください。'});
add('1-D',2,'曖昧な指示とアルゴリズム','コンピュータに「数字をいい感じに並べて」と指示しても正しく動かないのはなぜですか。アルゴリズムに必要な性質から理由を説明してください。',
 ['並べる基準（小さい順など）を決める','手順通りに並べ替える'],'「いい感じ」では基準（小さい順か大きい順か）や終了地点が曖昧だからです。アルゴリズムには明確で誰が見ても同じ結果になる具体的な手順が必要です。',null,{natural:true,answerFields:['正しく動かない理由'],aiQuestion:'問題1-Dについて質問です。「いい感じに並べて」という曖昧な指示ではコンピュータが処理できない理由について確認してください。'});
add('1-E',3,'変数と代入の役割','変数の役割と代入記号「←」の意味を、それぞれ説明してください。',
 ['箱（変数）を用意する','右辺の計算結果を箱へ入れる'],'変数は計算結果や入力された値を一時的に覚えておく「名前付きの箱」です。代入「←」は等しいという意味ではなく、「右辺の計算結果を左辺の箱へ入れる」という動作を表します。',null,{natural:true,answerFields:['変数の役割','代入「←」の意味'],aiQuestion:'問題1-Eについて質問です。変数の「値を保持する役割」と、代入記号「←」が等号ではないことの説明を確認してください。'});
add('1-F',3,'三つの基本構造の組み合わせ','「提出された答案用紙を1枚ずつ確認し、60点以上なら合格印、60点未満なら再試印を押し、全員分終わるまで続ける」という作業があります。三つの基本構造（順次・選択・繰返し）のうち、どれが含まれていますか。また、どの作業がどの構造に対応しているか説明してください。',
 [loop('未確認の答案がある',['答案を1枚確認する',branch('60点以上',['合格印を押す'],['再試印を押す'])])],'三つすべてが含まれています。答案を1枚ずつ確認する「繰返し」の中に、点数で印を分ける「選択」があり、一連の作業は上から順に進む「順次」で行われます。',null,{natural:true,answerFields:['含まれる基本構造','それぞれの対応関係'],aiQuestion:'問題1-Fについて質問です。答案確認の業務手順の中に順次・選択・繰返しがどのように組み合わさっているか確認してください。'});
...
add('8-D',1,'条件付きの更新','A＝{2,6}です。最後のresultを答えてください。',['result ← A[1]',branch('A[2] ＞ result',['result ← A[2]'])],'result＝6です。初期値2よりA[2]の6が大きいので更新します。',{result:6},{input:{A:[2,6]},answerFields:['最後の result'],aiQuestion:'問題8-Dについて質問です。初期値より大きい値が見つかった場合のみ変数を更新する処理のトレースを確認してください。'});
add('8-E',1,'二つの値の合計','A＝{3,5}です。最後のtotalを答えてください。',['total ← 0',repeat('i','1','2',['total ← total ＋ A[i]'])],'totalは0→3→8です。二つの要素を一回ずつ加算します。',{total:8},{input:{A:[3,5]},answerFields:['最後の total'],aiQuestion:'問題8-Eについて質問です。配列の全要素をループで走査して合計を計算する手順を確認してください。'});
add('8-F',2,'条件を満たした値だけ合計する','A＝{3,8,5,1}から5以上の値だけを集計します。count、total、averageを求めてください。averageは実数型です。この配列では対象が必ず一つ以上あります。',['count ← 0','total ← 0',repeat('i','1','4',[branch('A[i] ≧ 5',['count ← count ＋ 1','total ← total ＋ A[i]'])]),'average ← total ÷ count'],'count＝2、total＝13、average＝6.5。対象の8と5だけを足し、配列全体の4個ではなく対象の2個で割ります。',{count:2,total:13,average:6.5},{input:{A:[3,8,5,1]},types:'整数型: count, total, i\n実数型: average\n整数型の配列: A',answerFields:['count','total','average'],aiQuestion:'問題8-Fについて質問です。条件を満たす要素のみを集計し、対象件数で割って平均を求める手順を確認してください。'});
